...
const getErrorMessage = (error: unknown): string => {
  ...
  return "Unable to load the schedule.";
};
...
export default function AdminScheduleEditScreen() {
  ...
  const statusColors = getStatusColors(scheduleStatus);
  ...
  const loadScheduleData = useCallback(async (): Promise<void> => {
    ...
    if (!scheduleId) {
      setOptionsError("A valid schedule ID is required.");
    ...
    try {
      const [schedule, doctors, therapists] = await Promise.all([
        getScheduleById(scheduleId),
        getDoctors(),
        getTherapists(),
      ]);
      const loadedForm = scheduleResponseToForm(schedule);
      const nextDoctorOptions: SelectOption[] = doctors.map((doctor) => ({
        description: doctor.active
          ? doctor.specialization ?? undefined
          : `${doctor.specialization ?? "No specialization"} - Inactive`,
        id: doctor.id,
        label: doctor.name,
      }));
      const nextTherapistOptions: SelectOption[] = therapists.map(
        (therapist) => ({
      ...
      if (
        !nextDoctorOptions.some(
          (option) => option.id === schedule.doctor_id
        )
      ) {
        nextDoctorOptions.push({
          description: "Currently assigned",
          id: schedule.doctor_id,
          label: schedule.doctor_name ?? `Doctor #${schedule.doctor_id}`,
        });
      }

      if (
        !nextTherapistOptions.some(
          (option) => option.id === schedule.therapist_id
        )
      ) {
        nextTherapistOptions.push({
          description: "Currently assigned",
          id: schedule.therapist_id,
          label:
            schedule.therapist_name ??
            `Therapist #${schedule.therapist_id}`,
        });
      }

      setDoctorOptions(
        nextDoctorOptions.sort((first, second) =>
          first.label.localeCompare(second.label)
        )
      );
      setTherapistOptions(
        nextTherapistOptions.sort((first, second) =>
          first.label.localeCompare(second.label)
        )
      );
      setForm(loadedForm);
  ...
  const isDirty = useMemo(
    () =>
      ...
      getScheduleFormFingerprint(form) !==
        getScheduleFormFingerprint(originalForm),
    [form, originalForm]
  );

  usePreventRemove(
    (isDirty || submitting) && !navigationAllowed,
    ({ data }) => {
      if (submitting) {
        return;
      }
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    try {
      const schedule = await updateSchedule(scheduleId, request);

      Alert.alert(
        "Schedule Updated",
        `The schedule for ${schedule.patient_name} has been updated successfully.`,
        [
          {
  ...
  }, [form, goToScheduleList, handleSessionExpiry, params.id, validateForm]);
  ...
  const cancelForm = useCallback(() => {
    ...
    Alert.alert(
      "Discard Changes?",
      "The schedule changes entered on this form will be lost.",
      [
        {
          style: "cancel",
          text: "Keep Editing",
    ...
    <SafeAreaView edges={["top"]} style={styles.safeArea}>
      ...
      >
        <View style={styles.header}>
          <TouchableOpacity
            accessibilityLabel="Go back from schedule editing"
            accessibilityRole="button"
          ...
          <View style={styles.headerContent}>
            ...
            <View style={styles.titleRow}>
              ...
              <View
                accessibilityLabel={`Schedule status: ${scheduleStatus}`}
                accessible
                style={[
                  styles.statusBadge,
