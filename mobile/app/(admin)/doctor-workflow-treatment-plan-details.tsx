...
const cadenceFromFrequency = (frequency: string | null): number => {
...
export default function AdminTreatmentPlanDetailsScreen() {
  ...
  const [scheduleForm, setScheduleForm] = useState({ cadence_days: "1", date_mode: "treatment_date", in_time: "", instructions: "", number_of_sessions: "1", out_time: "", priority: "normal", session_date: getLocalIsoDate(), therapist_id: "" });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (error) => Alert.alert("Unable to Request Changes", getApiErrorMessage(error, "Unable to return this treatment plan for correction.")),
    onSuccess: async () => { await invalidate(); setPanel(null); setRejectionReason(""); Alert.alert("Changes Requested", "The plan was returned to the doctor with correction guidance."); },
  });
  ...
  const openSchedule = () => {
    ...
    setFormError(null);
    setScheduleForm({ cadence_days: String(cadenceFromFrequency(plan.frequency)), date_mode: "treatment_date", in_time: "", instructions: plan.special_instructions ?? "", number_of_sessions: String(plan.sessions_required || 1), out_time: "", priority: "normal", session_date: getLocalIsoDate(), therapist_id: "" });
    setPanel("schedule");
  };
  const submitReject = () => { if (!rejectionReason.trim()) { setFormError("Required corrections are needed."); return; } setFormError(null); rejectMutation.mutate(); };
  const submitSchedule = () => {
    const sessions = Number(scheduleForm.number_of_sessions);
    const cadenceDays = Number(scheduleForm.cadence_days);
    ...
    if (!Number.isInteger(cadenceDays) || cadenceDays < 1 || cadenceDays > 31) { setFormError("Days between sessions must be from 1 to 31."); return; }
  ...
  return (
    <SafeAreaView edges={["top", "bottom"]} style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl colors={[colors.primary]} refreshing={planQuery.isRefetching} tintColor={colors.primary} onRefresh={() => void planQuery.refetch()} />} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionsCard}><Text style={styles.actionsTitle}>Available actions</Text>{actions.has("approve") || actions.has("request_changes") ? <View style={styles.actionRow}>{actions.has("request_changes") ? <ActionButton disabled={busy} destructive icon="close-circle-outline" label="Request changes" onPress={openReject} /> : null}{actions.has("approve") ? <ActionButton disabled={busy} icon="checkmark-circle-outline" label="Approve" onPress={confirmApprove} /> : null}</View> : null}{actions.has("generate_schedule") ? <ActionButton disabled={busy} fullWidth icon="calendar-outline" label="Generate therapist schedule" onPress={openSchedule} /> : null}{generated ? <View style={styles.notice}><Ionicons color={colors.teal} name="checkmark-circle" size={20} /><Text style={styles.noticeText}>Schedule already generated</Text></View> : null}</View>
      </ScrollView>
...
function TimelineCard({ generated, plan }: { generated: boolean; plan: TreatmentPlan }) {
  const events = [{ icon: "add-circle-outline" as const, label: "Treatment plan created", timestamp: plan.created_at }, ...(plan.status === "approved" ? [{ icon: "checkmark-circle-outline" as const, label: "Treatment plan approved", timestamp: plan.reviewed_at ?? plan.updated_at ?? plan.created_at }] : []), ...(plan.status === "rejected" ? [{ icon: "return-down-back-outline" as const, label: "Changes requested", timestamp: plan.reviewed_at ?? plan.updated_at ?? plan.created_at }] : []), ...(plan.status === "submitted" && plan.revision > 1 ? [{ icon: "refresh-circle-outline" as const, label: "Corrected plan resubmitted", timestamp: plan.updated_at ?? plan.created_at }] : []), ...(generated ? [{ icon: "calendar-outline" as const, label: "Therapist schedule generated", timestamp: plan.updated_at ?? plan.created_at }] : [])];
